...
bundle = {
    ...
    "recommended_searches": [
        ...
        "cd /Users/sammytan/Developer/LiveMask/<target-repo> && <repo-native validation command>",
        "bash ../livemask-ci-cd/scripts/task-environment-freshness.sh --task-id <TASK-ID> --repo <repo>",
        "bash ../livemask-ci-cd/scripts/runtime-log-audit.sh --task-id <TASK-ID> --task-repo <repo>",
    ],
    "closure_reminders": [
        ...
        "Repo-native commands such as go test, npm test, flutter test, and go build must run from the target repo path, never from livemask-docs by accident.",
        "Local and remote runtime code freshness must be verified before coding and before completion.",
        "Container log errors must be classified as task-related or unrelated; unrelated errors need a GitHub issue link.",
    ],
}
