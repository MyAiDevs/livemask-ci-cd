...
def safe_docs_relative_path(docs_dir: pathlib.Path, relative_path: str) -> pathlib.Path | None:
...
def build_worker_brief(task: dict[str, Any], docs_dir: pathlib.Path) -> str:
    ...
    return "\n".join([
        ...
        f"- Module: {task.get('module_id', '')}",
        f"- Source task doc: {task_doc or '<none>'}",
        f"- Generated by: auto-task-assignment.py at {iso()}",
        "",
        "## Task Document",
        task_doc_content or "_No task document content was found._",
        "",
        "## Raw Task Metadata",
        "```json",
...
def sync_worker_brief(worker_repo: pathlib.Path, task: dict[str, Any], docs_dir: pathlib.Path) -> pathlib.Path:
    task_id = str(task.get("task_id", ""))
    brief_path = worker_repo / ".cursor-worker" / "briefs" / f"{task_id}.md"
